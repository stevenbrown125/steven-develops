...
export const SidebarProvider: React.FC<SidebarProviderProps> = ({
  ...
  const [isVisible, setIsVisible] =
    useState(true) /* Used to only when going from mobile to nonmobile */
  const [isMaximized, setIsMaximized] =
    useState(false) /* Used to toggle between nonmobile viewPoints */
  const [isDragging, setIsDragging] = useState(false)
  const [isAnimating, setIsAnimating] =
    useState(
      false,
    ) /* Used to prevent snappiness resulted from going full screen */
  ...
  const toggleSidebar = () => {
    ...
    if (viewWidth > MIN_WINDOW_BREAKPOINT) {
      /* Desktop */
      if (!isMaximized) /* We need to maximize it */ maximizeSidebar(viewWidth)
      else minimizeSidebar(viewWidth)
    } else {
      /* handle for mobile */
      if (isVisible) {
        closeSidebar()
  ...
  const minimizeSidebar = (viewWidth: number) => {
    ...
    else api.start({ width: SIDEBAR_MIN_WIDTH, immediate: false })
  }

  const updateSidebar = (viewWidth: number) => {
    if (isMaximized) {
      if (!isAnimating) api.start({ width: viewWidth, immediate: true })
  ...
  const bind = useGesture({
    ...
    onDrag: ({ movement: [mx] }) => {
      ...
      const newWidth =
        isMaximized || isAnimating
          ? Math.min(viewWidth, viewWidth + mx)
          : Math.max(sidebarWidth, sidebarWidth + mx)
      api.start({ width: newWidth, immediate: true }),
        { axis: "x", threshold: 150 }
    },
    ...
    onDragEnd: ({ movement: [mx], intentional, distance: [dx] }) => {
      ...
      if (intentional && dx > 30) {
        ...
        /* Wasn't intentional so reset sidebar state */
        if (isMaximized) api.start({ width: viewWidth, immediate: false })
        else api.start({ width: sidebarWidth, immediate: false })
      }
      setIsDragging(false)
    },
  })
